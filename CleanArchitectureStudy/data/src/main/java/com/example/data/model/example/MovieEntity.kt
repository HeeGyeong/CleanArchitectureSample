package com.example.data.model.example
 ...
 *          즉, MovieEntity 로 명명을 해두었지만, Api 는 변경 가능성이 높기 때문에 DTO 라고 생각해야 한다.
 *          DB 입장에서는 해당 값들이 변경되면 안되는 값들이기 때문에 Entity 로 어노테이션을 선언하여 사용한다.
 * DTO : 변경이 쉽게 가능한 Data Class.
 *        Api 와 같이 데이터 형태나 구조가 바뀔 수 있는 경우 사용하는 Data Class. DTO 를 가공하여 Entity 에 넣어 사용하기 때문에, 실제로 사용하는
